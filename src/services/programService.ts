...
import {
  ...
  export async function getCurrentProgram(): Promise<Program | null> {
    ...
    return {
      ...
      currentDay: data.currentDay as number,
   ...
   * updating only the `days` field so we don't clobber other props.
   */
  export async function saveAdaptedWorkout(
    ...
    days[dayIndex].exercises = adaptedExercises;
  
    // Only merge in the updated days array
